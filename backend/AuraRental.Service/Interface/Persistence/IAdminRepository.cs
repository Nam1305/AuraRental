...
public interface IAdminRepository
{
    ...
    Task<Branch?> GetBranchForUpdate(int branchId, CancellationToken cancellationToken);
    Task<bool> BranchCodeExists(string code, CancellationToken cancellationToken);
    Task<bool> BranchHasActiveWork(int branchId, CancellationToken cancellationToken);
    void AddBranch(Branch branch);

    Task<IReadOnlyList<User>> SearchUsers(string? query, bool? active, int limit, CancellationToken cancellationToken);
    Task<User?> GetUserForUpdate(int userId, CancellationToken cancellationToken);
    Task<IReadOnlyList<Branch>> GetBranchesByIds(IReadOnlyCollection<int> branchIds, CancellationToken cancellationToken);
